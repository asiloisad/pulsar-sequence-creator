...
module.exports = {
  ...
  simulate(value) {
    ...
    let simulateList = [];
    if (result !== null) {
      simulateList = __range__(0, this.simulateCursorLength - 1, true).map(
        (index) => {
  ...
  calculateNumberValue(
    index,
    { start, operator, step, radix, padString, digit, repeat, possign }
  ) {
  ...
  calculateAlphaValue(
    index,
    { start, operator, step, padString, digit, repeat }
  ) {
    index = parseInt(index / repeat);
    let count;
    switch (operator) {
  ...
  alphaSequence(str, count, upperCase) {
    ...
    let next = this.alphabetSequence[
      (index + count) % this.alphabetSequence.length
    ];
    if (!next) {
      return "";
    }
  ...
  leftPadding(str, digit, padString) {
    ...
    return (
      (fixCount ? firstLetter : "") +
      str.slice(fixCount).padStart(digit - fixCount, padString)
    );
  },
};
